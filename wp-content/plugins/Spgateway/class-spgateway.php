...
function spgateway_gateway_init() {
    ...
    class WC_spgateway extends WC_Payment_Gateway {
        ...
        function generate_spgateway_form($order_id) {
            ...
            $priceMonth = "499";
            $priceSeason = "399";
            $priceSeasonPackage = "1197";

            // $periodAmt
            if($order->get_total() == $priceSeasonPackage) {
                // Seanson members
                $periodAmt = $priceSeason;
            }
            else {
            ...
            // $periodType
            if($periodAmt == $priceWeek) {
                // Weekly
                $periodType = "W";
            }
            else if($periodAmt == $priceMonth || $periodAmt == $priceSeason) {
                // Monthly
                $periodType = "M";
            }
            else {
